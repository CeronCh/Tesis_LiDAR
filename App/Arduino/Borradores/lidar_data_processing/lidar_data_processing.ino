#include <HardwareSerial.h>
// Parámetros para configuración serial
#define RXD2 16
#define TXD2 17
const int LIDAR_BAUD = 230400;
const int MICRO_BAUD = 115200;
// Constantes para la longitud de los diferentes campos del paquete
const int packet_length = 47;                 // Longitud total del paquete por medicion
const int key_data_length = 28;               // Longitud del paquete con datos de medición relevantes

// Tabla de verificación de CRC proporcionada por los desarrolladores
static const uint8_t CrcTable[256]=
{ 
  
};


uint8_t CalCRC8(uint8_t *data_in, uint8_t len)
{
  uint8_t crc = 0;                            // Se inicializa la variable crc en 0
  for (uint16_t i = 0; i < len; i++) {        // Se recorre cada byte del array de datos apuntados 
    crc = CrcTable[(crc ^ *data_in++) & 0xff];// XOR entre crc y el valor del puntero que se incrementa en cada ciclo
  }
  return crc;
}

// Función para leer un paquete LiDAR y devolver verdadero cuando el buffer de datos es del tamaño y especificaciones indicadas
bool readLidar(uint8_t *data_in, size_t length) 
{
  if (Serial2.available() >= length) {        // Verifica que existan suficientes bytes disponibles para armar el paquete
    Serial2.readBytes(data_in, length);       // Almacena en el array dado, la cantidad de bytes indicados
    // Verifica los primeros dos indentificadores proporcionados por el desarrollador del paquete de información del LiDAR 
    // (Initial identifier = 0x54 y VerLen = 0x2C)
    if (data_in[0] == 0x54 && data_in[1] == 0x2C) 
    {
      return true;                           // Se leyó completamente un paquete
    }
  }
  return false;                              // No se leyó el paquete completo o los identificadores son incorrectos
}


void processPacket(uint8_t *data_in)
{
  uint8_t crc_calc = CalCRC8(data_in, packet_length - 1);
  uint8_t crc_rec = data_in[46];
  if(crc_calc == crc_rec)
  {
    uint8_t key_data[key_data_length];
    key_data[0] = data_in[4];                 // Byte alto del ángulo de inicio
    key_data[1] = data_in[5];                 // Byte bajo del ángulo de inicio
    // Se copian solo las distancias (se saltan los valores de intensidad)
    for (int i = 0; i < 12; i++)
    {
      key_data[2+i*2] = data_in[6+i*3];       // Byte alto de la distancia
      key_data[3+i*2] = data_in[7+i*3];       // Byte bajo de la distancia
    }
    key_data[26] = data_in[42];               // Byte alto del ángulo de inicio
    key_data[27] = data_in[43];                // Byte bajo del ángulo de inicio

    for (int i = 0; i < key_data_length; i++)
    {
      Serial.printf("%02X", key_data[i]);
    }
    Serial.println();
  }
  
}


void setup()
{
  // Se establece la comunicación en serie para visualizar en el monitor serial de la IDE lo que se envía
  Serial.begin(MICRO_BAUD);
  // Se establece la comunicación en serie entre LiDAR-ESP32
  Serial2.begin(LIDAR_BAUD, SERIAL_8N1, RXD2, TXD2);
}

void loop() {
  uint8_t buffer[packet_length];
  if (readLidar(buffer, packet_length)) {
    processPacket(buffer);
  }
}